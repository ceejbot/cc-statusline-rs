...
An opinionated ANSI statusline for Claude Code, in Rust. It reads the status-hook JSON on stdin and prints where you are, what the model is doing, and what it's costing you — on one colored line when your terminal is wide enough, split into identity and accounting lines when it isn't.
...
- A [Nerd Font](https://www.nerdfonts.com/) in your terminal. The line leans on Nerd Font glyphs; without one, it's a row of tofu.
- [Rust](https://rustup.rs/) 1.85 or later
- `git` on your PATH
- For the macOS install recipe: [just](https://just.systems/) and `jq` (`brew install just jq`)
